...
function init() {
 ...
 function renderCharityLi(charity) {
    ...
    charityLi.innerHTML = `<span class="nameSpan"> ${name} </span>     ||     <span class="einSpan"> EIN : ${ein} </span>  ||  <span class= "nteeSpan"> CODE/TYPE : ${ntee_code} </span>`
    charityLi.addEventListener("click", (e) => {
       console.log(`This Charity's name is ${name}, and EIN is ${ein}`)
       e.preventDefault()
 
       document.querySelector('h3.name').innerHTML = name
       document.querySelector('p.score').innerHTML = score
       console.log(ntee_code)
 ...
 function searchByName(searchTerm) {
    getData(0, searchTerm).then(data => data.organizations.forEach(renderCharityLi))
 }
   ...
   switch(type){
      case '0': case '1':
       ...
       case '4':
          ...
          break;
       case '5':
          document.querySelector('.charityType').src = 'https://independenttravelcats.com/wp-content/uploads/2019/12/Donation-boxes-National-History-Museum-3.jpg'
          break;
       case '6':
